...
function M.init()
    ...
    if res == 1 then
        ...
        return false, string.format(" од %d: %s", res, msg ~= "" and msg or "Ќеизвестна€ ошибка инициализации")
    end
end
...
---—оздает массив float4 на стороне CPU дл€ записи в буфер
---@param count integer
---@return ffi.cdata*
...
function M.create_buffer(size_bytes)
    ...
    assert(mem ~= nil, "[MoonCL] Ќе удалось выделить буфер в VRAM!")
...
function Buffer:write(cdata, size)
    assert(self.handle ~= nil, "[MoonCL] ѕопытка записи в освобожденный буфер!")
    local dll = get_dll()
...
function Buffer:read(cdata, size)
    assert(self.handle ~= nil, "[MoonCL] ѕопытка чтени€ из освобожденного буфера!")
    local dll = get_dll()
...
function Kernel:set_buffer(arg_idx, buffer_obj)
    ...
    ---@cast h ffi.cdata*
    assert(h ~= nil, "[MoonCL] ѕередан пустой или освобожденный буфер!")
